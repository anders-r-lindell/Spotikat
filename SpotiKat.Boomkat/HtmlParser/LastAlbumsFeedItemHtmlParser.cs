...
namespace SpotiKat.Boomkat.HtmlParser {
    public class LastAlbumsFeedItemHtmlParser : ILastAlbumsFeedItemHtmlParser {
        private const string PageNodesXPath = "//nav[@class='pagination']";
        private const string FeedItemNodesXPath = "//div[@id='wiselinks-container']//div[@class='product']";

        private const string ArtistNodeXPath =
            "div[@class='details']/div[@class='table table-fixed']/div[@class='table-row album']/div[@class='table-cell']/div/a/strong";

        private const string AlbumNodeXPath =
            "div[@class='details']/div[@class='table table-fixed']/div[@class='table-row album']/div[@class='table-cell']/div/a/span[@class='album-title']";
        ...
        private string GetAlbum(HtmlNode feedItemNode) {
            ...
            if (nodes != null && nodes.Count > 0) {
                return HttpUtility.HtmlDecode(nodes[0].InnerText).Replace("&apos;", "'");
            }
            return "";
        }
    }
